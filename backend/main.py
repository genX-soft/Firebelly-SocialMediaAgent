...
def _publish_to_facebook(account: SocialAccount, post: Post) -> tuple[str | None, str | None]:
  ...
  if post.media_url:
    ...
    print(f"DEBUG [FB Publish]: Posting text-only to {endpoint}")
  ...
  try:
    ...
    print(f"ERROR [FB Publish]: {exc.detail}")
    return str(exc.detail), None
  except Exception as exc:
    print(f"ERROR [FB Publish]: Unexpected: {exc}")
    return str(exc), None
...
def content_publish(payload: ContentPublishRequest, db: Session = Depends(get_db)): 
    ...
    # Mirror temporary DALL-E image URLs to Cloudinary for permanent storage
    media_url = payload.media_url
